...
// require library facebook SDk v4
		...
		if(count($data) > 0){
			if(isset($data['name']) && isset($data['email'])){
				...
				if(isset($resp['error'])){
					...
					if(!isset($res['error'])){
						$this->AccessApi->setAccess('in',(array)$res);
						echo json_encode(array('status' => true, 'message' => 'Berhasil masuk dengan facebook', 'data' => array('redirect' => site_url()))); die();
					} else {
						echo json_encode(array('status' => false, 'message' => 'Gagal masuk dengan facebook'));  die();
					}
				...
				echo json_encode(array('status' => false, 'message' => 'Tidak mendapatkan data dari facebook'));  die();
		}
		echo json_encode(array('status' => false, 'message' => 'Tidak bisa menghubung ke facebook'));  die();
	}
}